...
## Purpose
These examples show how mcp transport applies to real MCP engineering work.

## Practical example
A local code-search server uses stdio because the IDE launches it with workspace-specific settings. A shared compliance server uses Streamable HTTP with TLS, OAuth-backed authentication, session identifiers, request limits, and centralized logs.
