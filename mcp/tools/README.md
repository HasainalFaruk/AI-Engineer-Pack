...
## Purpose
Design executable MCP actions that models can request safely, with explicit schemas, permissions, observability, and result contracts.
...
## When to use
Use tools for searches, database queries, ticket creation, code changes, deployments, notifications, calculations, and other parameterized actions.

## When not to use
Do not use a tool for static context that can be exposed as a resource, or for vague commands whose behavior cannot be described and constrained.
