...
## Purpose
These examples show how mcp security applies to real MCP engineering work.

## Practical example
A deployment MCP server rejects production deploys unless the host presents a user token with release-manager scope and the user approves the exact service, version, and environment. Build logs are returned as untrusted context.
