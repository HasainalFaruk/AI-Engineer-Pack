...
## Purpose
These examples show how mcp clients applies to real MCP engineering work.

## Practical example
An IDE host connects to a local repository server over stdio and a project-management server over HTTP. It exposes read-only branch resources automatically, requires approval before creating issues, and blocks sampling requests unless the user enables them for the workspace.
